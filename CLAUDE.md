...
### 다음 세션 즉시 할 일
1. **Target API Level 명시적 34+ 설정** (1분)
   - `Project Settings → Player → Android → Other Settings → Target API Level` 이 현재 **Automatic (0)** 상태
   - Play Store 2024년 8월부터 API 34+ 강제. Automatic 의존 위험
   - 명시적으로 **34** 또는 **35** 로 잠가둘 것
2. **릴리즈 빌드 + 갤럭시 실기 end-to-end 검증**
...
3. **Firebase Crashlytics + Analytics 초기화** (30분)
...
5. **정식 앱 아이콘** — 현재 임시 (Assets/Media/NO2Icon.png 재사용)
