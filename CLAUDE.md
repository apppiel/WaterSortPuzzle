...
## 🔥 다음 세션 시작 시 (2026-07-22 밤 마감 시점)

### 최근 완료 (2026-07-22) — 🎉 지인 QA 피드백 반영 + App Store 정식 심사 제출

**최종 상태**: iOS 1.0 (2) 빌드 App Store Connect Upload 완료 + 심사 페이지 전부 작성 → **정식 심사 제출**. Apple 심사 대기 중 (평균 24-48h, 첫 앱이라 계정 신뢰도 검증 병행 시 며칠 걸릴 수 있음). 지인 TestFlight 재QA 는 스킵 (UI 3종 이슈는 지인 지적 그대로 반영이라 회귀 리스크 낮다는 판단).

**지인 QA 피드백 3종 fix (스크립트 편집)**:
- **UI 깨짐 (PlayButton/SettingsButton 흰 사각형)**: 프로젝트 복구 후 두 스프라이트 GUID 참조 (`1ea0836..`, `333356d..`) 가 소실. `_Heathen Engineering/Assets/UX/Icons/Flat Icons [Free]/` 에서 대체 아이콘 임포트 → `manage_components` 로 씬의 Image.sprite 를 새 GUID 로 세팅.
  - PLAY: `Free Flat Button Solid Triangle Icon.png` (사용자가 후에 직접 다른 걸로 변경)
  - SETTINGS: `Free Flat Gear 1 Icon.png`
- **버튼 히트박스 (텍스트 부분 안 눌림)**: `MainMenuManager.cs` 에 `ExpandClickToText` 추가. 텍스트 자식 밑에 `ClickArea` 라는 투명 Image + Button 을 stretch anchor + offset ±80/±60 로 생성해 클릭 영역 확장. 씬 구조 안 건드리고 런타임 코드만.
- **튜토리얼 강화 (힌트 무시하고 다른 조합 pour 성공 시 튜토리얼 조기 종료)**: `TutorialManager.RequiredTubeIndex` 프로퍼티 추가 (state 0→fromIdx, state 1→toIdx, state 2→null). `GameManager.HandleTubeClicked` 진입 시 이 값 체크 → 허용 안 되면 즉시 return. `StartTutorial` 시그니처에 `toIdx: 3` 파라미터 추가.

**Editor 흐름 차단 이슈 3건 fix**:
- **클리어 팝업 안 뜨는 버그** (Editor 만): macOS 에 Firebase 네이티브 (`FirebaseCppApp-13_13_0.bundle`) 소실 → `FirebaseAnalytics.LogEvent` 동기 호출 시 `DllNotFoundException` → 호출한 메서드 (Start, PlayClearSequence 등) 중단 → 팝업까지 도달 못 함. **fix**: 4군데 (`GameManager` 2, `RewardManager` 2) 모두 try-catch 로 감쌈. 실기는 정상 동작하므로 try 블록 실행. [[project-editor-firebase-missing-native]]
- **AppBootstrap Firebase 예외** (Editor 만): 게임 흐름엔 영향 없어서 방치. 실기 초기화 실패 시 감지용으로 유지 (사용자 판단).
- **Editor 에서 리셋 무반응**: `AdManager.ShowRewarded` 가 씬 리로드 (onRewarded) 후 광고 Show → Editor AdMob 시뮬레이터가 씬 전환과 충돌해 뜨자마자 꺼지고 input lock. **fix**: `#if UNITY_EDITOR` 로 광고 Show 자체 skip. 실기엔 원래 순서 그대로 (리셋 → 광고).

**iOS 재빌드 → TestFlight → 심사 제출 흐름**:
- Unity iOS 재빌드 (Build Number `2`, Version `1.0` 유지) → `~/Desktop/project builds/IOS_Builds3/`
- Podfile 수동 편집: `IOS_Builds2/Podfile` 을 그대로 `cp` 로 덮어쓰기 → `pod install`. `use_frameworks!` (dynamic) + `post_install` hook 자동 적용. [[project-podfile-manual-fixes]] 여전히 유효.
- Xcode `.xcworkspace` → Manual Signing 재설정 → Archive → Distribute App → App Store Connect Upload → 서버 processing 완료
- App Store Connect 웹에서 심사 페이지 작성:
  - 프로모션 텍스트: 스킵 (선택 사항)
  - 설명: 오프닝 후크 + 게임 방법 + 특징 + 리워드 이벤트 + 타겟 유저 구조 (Play Console 톤 통일). "광고 없이" 문구 있으면 유저 오해 + 리뷰 폭탄 리스크 → 광고 있음 명시.
  - 부제 (Subtitle, 최대 30자): 검색 알고리즘에 반영되므로 넣는 게 유리. 재심사 필요.
  - 키워드, 지원 URL (`https://nanabox.co.kr`), 개인정보 처리방침 URL (`https://nanabox.co.kr/privacy.html`)
  - 앱 심사 정보: 연락처 (사장님 실명/전화/이메일 `nanabox79@gmail.com`), 로그인 필요 = 아니오
  - **ITSAppUsesNonExemptEncryption**: 2번째 옵션 "Apple 시스템 암호화 + 표준 알고리즘" 선택 → 후속 "면제 자격 있음? 예" → "프랑스 배포? 아니오" 로 자동 면제
  - 데이터 수집 (Play Console 신고와 일치): 식별자>기기ID (트래킹 예), 사용 데이터>제품 상호작용 (트래킹 아니오), 사용 데이터>광고 데이터 (트래킹 예), 진단>충돌 데이터 (트래킹 아니오), 진단>실적 데이터 (트래킹 아니오). 모두 사용자 신원 연결 = 예 (Firebase pseudonymous ID 로 자동 연결).
  - 연령 등급: 4+ 자동 계산. **"어린이용 (Kids Category)" 절대 선택 금지** (AdMob 실질 불가 + Firebase 제약 + COPPA 강제).
  - 콘텐츠 권한: 아니오 (게임 자체 콘텐츠 + 에셋 스토어 라이센스는 자체 사용권한). 광고는 별도 신고했으니 중복 언급 X.
  - 가격: 무료. 지역: 대한민국만 (UMP SDK 안 넣었으므로 EU 배포 시 AdMob 계정 정지 리스크).
- **심사 제출 완료**. 결과 대기.

### (이전) 최근 완료 (2026-07-20 밤) — 🎉 iOS Archive 3중 이슈 다 뚫고 TestFlight 업로드 완료
...
**핵심**: iOS 정식 심사 제출 완료 → 결과 대기. Play Console 스토어 페이지 아이콘 재업로드 정리. NO.1 심사 결과 확인.

0. **시작 시 상태 sanity check** (30초)
   - Apple ID 이메일 확인 → App Store Connect 심사 결과 알림 (승인 or 반려)
   - App Store Connect → 나나박스 워터소트 → 앱 상태 확인: "심사를 위해 제출됨" → "심사 중" → "판매 준비 완료" or "메타데이터 거부됨" 등
   - Play Console (NO.1 한붓그리기) 심사 상태 확인
   - Unity 콘솔 에러 0 확인 (Firebase Editor 경고는 정상)

1. **🔴 최우선: iOS 심사 결과 대응**
   - **승인 시**: 자동 배포 활성화 상태면 App Store 에 바로 등장. 지인·주변에 링크 공유해서 실기 다운로드 검증. 첫 실행 시 ATT 팝업 (`NSUserTrackingUsageDescription`) 뜨는지 확인.
   - **반려 시**: 이메일에 사유 명시됨. 대개:
     - 스크린샷 규격 안 맞음 → iPhone 6.7" (1290x2796) 재업로드
     - 개인정보 처리방침 URL 접근 불가 → 페이지 실제 접근 가능한지 확인
     - 광고 관련 오해 소지 문구 → 설명·부제 수정
     - ATT 팝업 문구 애매 → `NSUserTrackingUsageDescription` 값 재확인
   - Resolution Center 에서 답변 → 필요 시 수정 후 재제출 (버전 유지 or 빌드 재업로드)

2. **Play Console (NO.2 안드로이드) 스토어 아이콘 재업로드**
...
3. **App Store Connect 스토어 페이지 아이콘 (1024×1024)** 도 심사 통과 후 재업로드 필요할 수 있음 (앱 번들 아이콘과 별개).

4. **NO.1 (한붓그리기) Play Console 심사 결과 확인 → NO.2 도 심사 요청**
   - NO.1 승인 시 → Play Console NO.2 도 "검토를 위해 저장" 클릭

5. **정식 App Store 심사 승인 후 후속** (승인 시)
   - AdMob iOS 앱 상태가 "Ready" 로 바뀌는지 확인 (App Store 정식 게시 후 24~72h)
...
6. **다음 iOS 빌드 세션 시 재적용 필요** (Podfile 수동 편집 유실 방지)
   ...
   - 편한 방식: 이전 빌드의 Podfile 을 새 빌드 폴더에 `cp` 로 덮어쓰기 후 `pod install` (이번 세션에 검증됨).
   - 근본 해결: `Assets/Editor/iOSPostBuildProcessor.cs` 에 Podfile 편집 후처리 로직 추가하면 자동화 가능 (선택). [[project-podfile-manual-fixes]] 참조.

7. **(이전 마감 이슈로 남아 있던) TestFlight Internal Testing 그룹**
