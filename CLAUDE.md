...
### 다음 세션 즉시 할 일
1. **새 빌드 실기 재검증** — 이번 + 이전 세션 변경 사항 총 5건 한번에
   - Release 빌드 (Development Build 해제) → 갤럭시 재설치
   - **튜토리얼**: Level 1 첫 진입 시 손 등장, Tube 1 눌러야 이동, pour 후 사라짐
   - 튜브 크기: 4~5 튜브 레벨 자연스러운지 (segSize 0.62)
   - 리셋 버튼: 광고 뜨면 시청 후 리셋, 안 뜨면 즉시 리셋
   - 전면 광고 게이팅: 레벨 3/6/9 클리어 시에만 다음 레벨 클릭에서 광고
   - Firebase 실기 데이터: Crashlytics 활성 유저 + Analytics 이벤트 도착
2. **AdMob 실제 광고 확인** (며칠 후) — 새 앱 등록 24-72시간 검증 대기 중. 지금 안 나오는 건 정상.
3. **Firebase 실기 데이터 확인**
   - Crashlytics 대시보드: 활성 유저/세션 카운트
   - Analytics DebugView: `adb shell setprop debug.firebase.analytics.app com.nanabox.watersortpuzzle` 후 앱 실행 → `level_start`, `level_clear`, `reward_issued` 실시간 확인
4. **정식 앱 아이콘** — 현재 임시 (Assets/Media/NO2Icon.png 재사용). Play Console 압축 경고 뜸.
5. **Play Console 최종 등록**
   - 개인정보처리방침 URL: `https://nanabox.co.kr/privacy.html` (갱신본 배포 완료)
   - 앱 아이콘 정식본, 스크린샷 (여러 해상도)
   - Release AAB 업로드
   - **배포 지역 = 한국 only** 확인 (UMP 스킵의 전제)
