...
### 최근 완료 (2026-07-14)
- **100 레벨 전부 완성** + 갤럭시에서 **첫 실기 빌드 성공**
- **메인 메뉴 UI 정렬 (모든 폰 비율 대응 완료)**
  - `MainMenuScene`: Background Image `preserveAspect=true` 로 스프라이트 왜곡 방지
  - PlayButton/SettingsButton 앵커·피벗 (0.5, 0.5), anchoredPosition `(-299, -44)` / `(-299, -368)`
  - **PinkFill** GameObject 추가 (Canvas의 첫 자식): stretch full canvas + Image color `#FFC3C3` (스프라이트 픽셀 샘플링). 어떤 폰 비율(9:16~9:22)이든 letterbox 영역이 pink로 자연스럽게 채워짐 (파란 카메라 clear 노출 X).
- **AdManager 스레드 버그 수정** (`Assets/Scripts/Game/AdManager.cs`)
  - `Debug.isDebugBuild`는 non-main thread에서 호출 금지 → `MobileAds.Initialize` 콜백에서 property로 호출하면 UnityException 발생
  - Awake(메인 스레드)에서 `_interstitialAdUnitId` / `_rewardedAdUnitId` 필드에 캐싱하도록 변경
- **Background/PinkFill 픽셀 값** = `#FFC3C3` = 브랜드 라이트 핑크 `#FFC2C2`와 사실상 동일

### 다음 세션 즉시 할 일
1. **릴리즈 빌드 검증** (5분)
   - Build Settings에서 `Development Build` **해제**
   - Android 빌드 → 갤럭시에 재설치
   - 확인: (a) Console 오버레이 배너 사라졌는지, (b) UI 통 pink로 잘 나오는지, (c) AdMob 실제 광고 나오는지 (로드 실패해도 크래시는 X여야 함)
2. **UMP SDK (GDPR 동의 UI)** — EU 유저 대응, AdMob 정책상 필수
3. **정식 앱 아이콘** — 현재 임시 (Assets/Media/NO2Icon.png 재사용 중)
4. **개인정보 처리방침 URL** — Play Console 필수
5. **리워드 코드 시스템** — Level 100 클리어 시 Firebase Firestore로 코드 발급, 실물 상품 교환. No1 게임 참고 코드 있음. `RewardManager.cs` 신규 필요.
