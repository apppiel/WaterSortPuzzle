...
- **실기 검증 (갤럭시 S22+) 통과 항목**
  - 재설치 감지 → 레벨 1 초기화 ✓
  - 리워드 팝업 표시 + 코드 발급 + nana-no2 Firestore 저장 ✓
  - 홈페이지 reward-claim.html 에서 NO.2 코드 조회/신청 ✓
  - 테스트 광고 (Development Build) 정상 노출 ✓
  - **미검증**: 실제 광고 (AdMob 새 앱 검증 대기 24-72시간), 새 빌드의 튜브 크기·리셋 fix·3라운드 게이팅
- **실기 UX 이슈 3건 수정** (`GameManager.cs`, `AdManager.cs`)
  - 튜브 크기: `segSize 0.7 → 0.62`, `spacing 1.1 → 0.98` (4개 이하 레벨 — 예전엔 5+ 레벨보다 유독 커 보였음)
  - 리셋 버튼: 보상형 광고 실패해도 리셋 실행. 기존엔 광고 미준비 시 Debug.Log만 찍고 아무 반응 없어 "버튼 안 눌리는" 버그처럼 보였음 → 광고 성공/실패 모두 `DoReset` 호출로 변경
  - HUD 버튼 재배치 (Inspector 수동, 사용자가 SafeArea 이해도 올린 후)
- **전면 광고 3라운드 게이팅** (`AdManager.ShowInterstitialGated`)
  - 기존엔 매 레벨 클리어마다 광고 시도 → 유저 이탈 우려
  - 세션 카운터로 3번째 요청에서만 실제 노출 (레벨 3, 6, 9, ...)
  - 리셋의 보상형 광고는 별개 (게이팅 없음)
- **실기 Firebase config 심각 버그 fix** (2026-07-15)
  - `Assets/Plugins/Android/FirebaseApp.androidlib/res/values/google-services.xml` 이 NO.1(nana-53f2e) 값 그대로였음 → 실기 리워드 코드가 NO.1 DB로 저장되던 사고
  - EDM4U는 이 XML이 이미 있으면 재생성 안 함. NO.1 폴더 복사 시 딸려온 XML을 놓쳤음
  - 값을 nana-no2 로 수동 교체 완료. Editor 테스트로 못 잡힘 (Editor는 `google-services-desktop.json` 참조) — 실기 검증에서만 발견됨
- **홈페이지 두 프로젝트 병렬 조회 지원** (NO.1 리포의 `Cafe24/reward-claim.html`)
  - 웹 SDK named app 으로 nana-53f2e + nana-no2 동시 초기화
  - `code_index` 를 두 DB 병렬 조회 → 발견된 쪽 DB에 claims 저장, `game: "NO.1"/"NO.2"` 필드로 구분
  - NO.1 리포에서 커밋 (WaterSortPuzzle 리포엔 반영 X)
- **Firebase Crashlytics + Analytics 세팅 완료**
  - Firebase Console에서 Crashlytics 활성화, Google Analytics 링크(Default Account for Firebase 재사용)
  - Crashlytics SDK 임포트 (`Assets/Firebase/Plugins/Firebase.Crashlytics.dll`) — Firebase Unity SDK 13.13.0 zip에서 `FirebaseCrashlytics.unitypackage`
  - `AppBootstrap.cs` 신규 — `RuntimeInitializeOnLoadMethod(BeforeSceneLoad)` 로 앱 시작 시 Firebase 초기화 + `Crashlytics.ReportUncaughtExceptionsAsFatal = true` (Unity unhandled exception 을 fatal 로 리포트)
  - 커스텀 Analytics 이벤트 3개: `level_start(level)`, `level_clear(level, duration_sec)`, `reward_issued(is_reissue)`. 어느 레벨이 어렵고, 얼마나 걸리고, 리워드 발급 카운트를 대시보드에서 확인 가능.
  - **실기 검증 필요**: 앱 실행 → Firebase Console 에서 이벤트 도착 확인 (Analytics 는 몇 시간 지연, 실시간은 DebugView 사용).
...
### 다음 세션 즉시 할 일
1. **새 빌드 실기 재검증** (튜브 크기/리셋 fix/3라운드 게이팅)
   - Release 빌드 (Development Build 해제) → 갤럭시 재설치
   - 4~5 튜브 레벨: 튜브 크기 자연스러운지 (0.62)
   - 리셋 버튼: 광고 뜨면 시청 후 리셋, 안 뜨면 즉시 리셋 (앱 켜자마자도 동작)
   - 레벨 3, 6, 9 클리어 시 다음 레벨 클릭에서만 전면 광고 뜨는지
2. **AdMob 실제 광고 확인 (며칠 후)** — 새 앱 등록 후 24-72시간 검증 대기. 그 후 자연스레 실제 광고 서빙 시작. 지금 안 나오는 건 정상.
3. **Firebase 실기 데이터 확인**
   - Crashlytics 대시보드: 활성 유저/세션 카운트 뜨는지
   - Analytics DebugView: `adb shell setprop debug.firebase.analytics.app com.nanabox.watersortpuzzle` 후 앱 실행 → 이벤트 실시간 확인 (`level_start`, `level_clear`, `reward_issued`)
4. **UMP SDK** (GDPR 동의 UI) — EU 유저 대응, AdMob 정책상 필수
5. **정식 앱 아이콘** — 현재 임시 (Assets/Media/NO2Icon.png 재사용). Console에도 압축 경고 뜸.
6. **개인정보 처리방침 URL** — Play Console 필수
7. **튜토리얼** — 첫 레벨 손가락 가이드. 리텐션 결정적.
...
- **리셋 광고 실패 → 리셋 실행 정책** (의도적): `HandleReset` 은 광고 로드 실패해도 무조건 리셋 실행. 광고 수익 손해 미미 vs "버튼이 안 눌린다" UX 리스크가 훨씬 큼. 앱 켠 직후 리셋 눌러도 반응 있어야 유저 신뢰 유지.
...
- [x] 보상형 광고 — 리셋 버튼 (**광고 실패해도 리셋 실행**, UX 우선)
...
- [x] Firebase Crashlytics + Analytics — 프로덕션 크래시/유저 행동 트래킹 (`AppBootstrap.cs`, `level_start`/`level_clear`/`reward_issued` 이벤트)
