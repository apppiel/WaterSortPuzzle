...
## 🔥 다음 세션 시작 시 (2026-07-23 밤 재마감 시점)

### 최근 완료 (2026-07-23 밤) — 팝업 3종 라운드 코너 + iOS 1.0.1 심사 통과 (07-24 10:34 배포)

**팝업 3종 라운드 코너**:
- Menu / Clear / Reward 팝업의 배경/버튼/코드박스에 살짝 둥근 모서리 적용
- `Assets/Scripts/Game/PopupHelpers.cs` 신설 — `AddRoundedImage(GameObject, Color)` 공통 유틸
- Unity 6 에선 `Resources.GetBuiltinResource<Sprite>("UI/Skin/UISprite.psd")` 내장 리소스 경로가 사라져 fallback → **프로그래매틱으로 흰 라운드 사각형 텍스처 생성** (32×32 안 26×26, 알파 마스크 + 1px 페더링, 9-slice border=Radius)
- 반경 튜닝: 12 → **15px** (유저 조정). `PopupHelpers.cs:13` `Radius` 상수 하나로 세 팝업 전체 일괄 제어
- 오버레이(전체 화면 어둡게) 는 라운딩 제외 — 스크린 엣지에 걸쳐 있어 라운드 무의미
- [[project-editor-firebase-missing-native]] 관련 로그는 무관 (기존 try-catch 로 흐름 유지)

**iOS 1.0.1 (Build 3) 재빌드 → App Store 제출 → 심사 통과**:
- 이전 빌드 (`IOS_Builds3/`) Podfile `cp` 로 새 빌드 폴더 (`IOS_Builds4/` 로 추정) 덮어씀 → `pod install` [[project-podfile-manual-fixes]]
- Xcode Archive → Distribute App → App Store Connect Upload → processing 완료
- App Store Connect: v1.0 (2) 승인 판정 후 승인 → **v1.0.1 새 버전 생성** → 빌드 3 선택 → 새로운 소식 작성 → 심사 제출
  - "새로운 소식" 문구: "게임 안정성 개선 / 팝업 UI 소소한 개선" (허점 언급 X)
  - ITSAppUsesNonExemptEncryption: **2번 (표준 암호화 알고리즘, Apple OS 병행 사용)** → 자동 면제. 세부 4지선다는 첫 접수 (1.0 (2)) 이후 자동 프리셋 되어있음
- **심사 결과**: 반나절 컷 통과 (metadata 미변경 + 계정 신뢰도 덕). **07-24 10:34 부터 App Store 실효**
- 심사 취소 없이 직접 새 버전 제출 가능했던 이유: v1.0 (2) 가 이미 승인 완료됐던 상태

### 최근 완료 (2026-07-23) — 리셋 광고 우회 허점 차단

유저가 발견: **인게임(진행 중) → 메뉴 → 레벨선택 → 아무 레벨 재진입** 하면 초기 상태로 리셋되면서 `HandleReset` 이 요구하는 보상형 광고를 완전히 우회. NO.2 수익 누수 케이스.

**초안 (게이팅 위치: 메뉴 → LS 클릭 시점)**: `GameManager.HandleMenuLevelSelect` 에서 `_resetButton.interactable == true` 면 `ShowRewarded(GoLevelSelect)`. 유저 검토 결과 방향 재조정 요청 — 게이팅 지점을 LS→Game 진입으로 옮기자.

**최종 (게이팅 위치: 다음 게임 씬 진입 시점)**:
- `AdManager.PendingRewardedOnLevelEnter` static bool 추가 (process 스코프, 앱 재시작 시 자연 리셋, PlayerPrefs 아님)
- `GameManager.HandleMenuLevelSelect`: pour 이력 있으면 (`_resetButton.interactable`) 플래그 세팅 후 LS 로드
- `GameManager.Start` 마지막: 플래그 세팅돼있으면 소비 + `ShowRewarded(() => {})` 호출 → 광고가 새 게임 씬 위로 뜸

**왜 static 플래그 방식**: `LevelSelectManager` 는 `WaterSortPuzzle.UI` asmdef 라 `WaterSortPuzzle.Game` (AdManager 위치) 참조 불가 (circular 됨). LS 씬을 건드리지 않고 GameManager 안에서 signal 을 발신/수신하는 게 asmdef 경계 존중 + 최소 침습.

**게이팅 판정 = pour 성공 여부**: 리셋 버튼 활성화 조건 (`_resetButton.interactable`) 재사용. pour 없이 메뉴만 열었다 닫으면 광고 예약 안 됨 — 리셋할 상태가 없으니 광고도 없다는 논리.

**허점 커버리지**: 어느 레벨로 재진입해도 광고 (레벨 비종속). "다른 레벨로 우회" 트릭까지 차단.

**실기 검증 완료** (Android Development Build, 테스트 광고 표시 확인).

### 최근 완료 (2026-07-22 밤) — Android v1.1 시도 → keystore 지문 불일치 → upload key reset 신청

07-19 원본 keystore 소실 후 재생성한 사실을 v1.1 AAB 업로드 시도에서 Play 서버가 거부하며 실감. 예상됐던 시나리오 ([[project-keystore-replaced]] 미리 기록) 라 즉시 대응.

**한 것**:
- Play Console → 앱 → **Google Play로 보호됨** → Play 스토어 보호 → 앱 서명 키 보호 → **Play 앱 서명 관리** → 업로드 키 재설정 요청
- 사유: "업로드 키를 분실함"
- 새 keystore PEM 추출 (macOS 시스템 Java 없어서 Unity 번들 keytool 사용):
  ```
  /Applications/Unity/Hub/Editor/6000.3.13f1/PlaybackEngines/AndroidPlayer/OpenJDK/bin/keytool \
    -export -rfc -keystore /Users/choiseungjin/letme/unity_/Keystore/NANA_user.keystore \
    -alias nana_key -file ~/Desktop/upload_certificate_wsp.pem
  ```
- 새 인증서 SHA1 (`EF:A4:B7:99:03:73:A1:A8:22:62:6C:FA:6B:48:6C:E5:97:58:9A:C8`) = Play 거부 메시지의 새 지문과 일치 확인
- 제출 완료. **NO.1 (한붓그리기) 도 같은 절차로 별도 신청 완료** (파일명 `upload_certificate_no1.pem`).

**대기**: Google 승인 평균 2~5 영업일. 승인 나오기 전엔 v1.1 업로드 불가.

**iOS 상태**: "1.0 심사 대기 중" (정상 접수). NO.1 이 이미 App Store 게시돼있어 계정 신뢰도 재검증 불필요 → 예상 심사 24~48h.

### 최근 완료 (2026-07-22) — 지인 QA 피드백 반영 + App Store 정식 심사 제출

iOS 1.0 (2) 빌드 App Store Connect Upload + 심사 페이지 작성 → **정식 심사 제출**. 지인 재QA 는 스킵 (지적 그대로 반영이라 회귀 리스크 낮음).

**지인 QA 피드백 3종 fix**:
- **UI 깨짐 (PlayButton/SettingsButton 흰 사각형)**: 프로젝트 복구 후 스프라이트 GUID 소실. `_Heathen Engineering/Assets/UX/Icons/Flat Icons [Free]/` 에서 대체 아이콘 임포트 → `manage_components` 로 씬 Image.sprite 갱신.
- **버튼 히트박스 (텍스트 영역 안 눌림)**: `MainMenuManager.cs` `ExpandClickToText` 추가. 텍스트 자식 밑에 stretch anchor + offset ±80/±60 로 투명 `ClickArea` Image + Button 런타임 생성.
- **튜토리얼 조기 종료 방지 (힌트 무시 pour 성공 시)**: `TutorialManager.RequiredTubeIndex` 추가 (state 0→fromIdx, 1→toIdx, 2→null). `GameManager.HandleTubeClicked` 진입 시 체크 → 허용 안 되면 즉시 return.

**Editor 흐름 차단 이슈 3건 fix**:
- **클리어 팝업 안 뜨는 버그** (Editor 만): Firebase 네이티브 (`FirebaseCppApp-13_13_0.bundle`) 미탑재 → `LogEvent` 동기 호출 시 `DllNotFoundException` → 호출 메서드 중단. **fix**: 4군데 (`GameManager` 2, `RewardManager` 2) try-catch. [[project-editor-firebase-missing-native]]
- **AppBootstrap Firebase 예외** (Editor 만): 게임 흐름 영향 없어 방치.
- **Editor 리셋 무반응**: `AdManager.ShowRewarded` 광고가 씬 리로드와 충돌해 input lock. **fix**: `#if UNITY_EDITOR` 로 광고 Show 자체 skip. 실기 순서 유지.

**iOS 재빌드 → 심사 제출 흐름**:
- Unity iOS 재빌드 (Build Number `2`, Version `1.0` 유지) → `~/Desktop/project builds/IOS_Builds3/`
- Podfile 수동 편집: `IOS_Builds2/Podfile` `cp` 로 덮어쓰기 → `pod install`. `use_frameworks!` (dynamic) + `post_install` hook 자동 적용. [[project-podfile-manual-fixes]]
- Xcode Manual Signing → Archive → Distribute App → App Store Connect Upload → processing 완료
- App Store Connect 심사 페이지 작성 → 제출:
  - **ITSAppUsesNonExemptEncryption**: "Apple 시스템 암호화 + 표준 알고리즘" → 자동 면제
  - 데이터 수집 (Play Console 신고와 일치): 기기ID·광고데이터 트래킹 예, 나머지 아니오. 사용자 신원 연결 모두 예 (Firebase pseudonymous ID).
  - **"어린이용 (Kids Category)" 절대 금지** (AdMob 실질 불가 + Firebase 제약)
  - 설명·부제 "광고 없음" 뉘앙스 금지. 리워드 이벤트 명시.
  - 가격 무료, 지역 대한민국만 (UMP SDK 미탑재)

### 최근 완료 (2026-07-20) — iOS 이식 + Xcode Archive 3중 이슈 뚫고 TestFlight 업로드

**iOS 이식 준비**:
- Apple Developer Bundle ID (`com.nanabox.watersortpuzzle`, Team `X9W8QX2MMN`), App Store Connect 앱 생성
- Firebase iOS `GoogleService-Info.plist` (프로젝트 `nana-no2`) → `Assets/` 루트
- AdMob iOS App ID (`ca-app-pub-3079888946602647~6304172559`) `GoogleMobileAdsSettings.asset` 설정
- iOS 아이콘 19개 슬롯 프로그래매틱 `NO2Icon` 세팅
- `Assets/Plugins/iOS/ATTBridge.mm` 신규, `iOSPostBuildProcessor.cs` 확장 (`GADApplicationIdentifier` + `NSUserTrackingUsageDescription` 삽입 + `AppTrackingTransparency.framework` 링크)
- `AdManager.cs` iOS ATT 초기화 로직 (콜백에서 `MobileAds.Initialize`)
- EDM4U `SwiftPackageManagerEnabled=false` [[project-podfile-manual-fixes]]
- 아이콘 재디자인 (`NO.2` 랭킹 오해 방지 → "나나박스 games / water sort puzzle")

**Archive 3중 이슈 순차 해결**:
1. **SPM Firebase 이중 정의** (`Redefinition of module 'Firebase'`): Xcode Package Dependencies 에서 `firebase-ios-sdk` 수동 제거
2. **`Multiple commands produce Metadata.appintents`**: Podfile `use_frameworks!` (dynamic) + `post_install` hook (App Intents 무효화)
3. **Firebase iOS 네이티브 `.a` 3개 누락**: `Assets/Plugins/iOS/Firebase/` 에 Analytics/Auth/Firestore 없어서 링크 실패. NO.1 에서 복사. Editor managed DLL 만으로 컴파일 통과해 여태 안 잡혔던 이슈. [[project-firebase-ios-native-libs]]

**iOS Signing**: Manual (App Store Connect 프로파일). Automatic 은 등록 디바이스 없으면 Archive 블록. [[ios-signing-needs-udid]]

### 최근 완료 (2026-07-19) — 프로젝트 폴더 소실 → 복구

- GitHub clone 으로 소스 복구, Unity Library/ 재생성 (에러 0)
- **배포 keystore 원본 완전 소실** → Unity Keystore Manager 로 재생성. 경로·alias(`nana_key`) 동일 유지. Google Drive 백업. [[project-keystore-replaced]]
- 심사 중 v1.0 에는 영향 없음. v1.1+ 낼 때 Play upload key reset 신청 필요 → **07-22 밤 실제 필요해져 신청 완료**.
...
**핵심**: iOS 는 v1.0.1 배포 대기만 남음. Android 는 upload key reset 승인 + NO.1 심사 결과 대기.

0. **시작 시 상태 sanity check** (30초)
   - Apple ID 이메일 (`nanabox79@gmail.com`) 확인:
     - Play Console upload key reset 승인 알림 (NO.1·NO.2 별개)
   - App Store Connect → 나나박스 워터소트 상태: **07-24 10:34 이후엔 "판매 준비 완료"** 로 뜰 것. 실기(iPhone) 업데이트 확인
   - Play Console NO.1 v1.0 심사 상태 (07-18 재업로드, 지금 6일차 이상)
   - Unity 콘솔 에러 0 확인 (Firebase Editor 경고는 정상)

1. **🟢 iOS v1.0.1 실기 검증** (07-24 10:34 이후)
   - iPhone 자동 업데이트 → 실행
   - **리셋 광고 우회 차단 확인**: 인게임 pour → 메뉴 → 레벨선택 → 아무 레벨 클릭 → 광고 뜨는지
   - **팝업 라운드 확인**: Menu/Clear/Reward 팝업 모서리 살짝 둥근지 (Reward 는 100 레벨 클리어 필요 → Editor 로 재현 or 계정 데이터 조작 필요)
   - 첫 실행 ATT 팝업 (`NSUserTrackingUsageDescription`) 문구 확인
   - AdMob 실제 광고 검증: 24~72h 후 실기에서 테스트 광고 아닌 실제 광고 배너 뜨는지

2. **🔴 Play upload key reset 승인 대응** (NO.1·NO.2 별개, 07-22 밤 신청)
   - 승인 나면 v1.1 AAB 재업로드 → 지문 통과할 것
   - Unity Player Settings → **Bundle Version Code +1, Version Name `1.0.1`** 로 갱신 후 Release AAB 빌드 (Development Build 해제)
   - 이번 세션 변경(리셋 우회 차단 + 팝업 라운드) 포함된 상태로 나감

3. **NO.1 Play v1.0 심사 결과 확인 → NO.2 도 심사 요청**
   - NO.1 07-18 재업로드 후 심사 중 (07-24 기준 6일차 이상)
   - NO.1 승인 → Play Console NO.2 draft 도 (v1.1 AAB 교체 후) "검토를 위해 저장" 클릭

4. **Play Console NO.2 스토어 페이지 아이콘 재업로드**
   - 스토어 등록정보 → 앱 아이콘 슬롯 새 `NO2Icon.png` (또는 512×512 리사이즈본). 앱 번들 아이콘과 별도 관리라 스토어 페이지엔 여전히 `NO.2` 남아있을 수 있음.

5. **App Store Connect 스토어 아이콘 (1024×1024)** — 필요 시 재업로드.

6. **정식 App Store 배포 후 후속**
   - AdMob iOS 앱 상태 "Ready" 확인 (24~72h)
   - Firebase Analytics 실기 이벤트 도착 확인
   - 첫 리뷰 도착 시 대응

7. **다음 iOS 빌드 세션 시 재적용**
   - 이전 빌드 Podfile `cp` 로 새 빌드 폴더 덮어쓰기 후 `pod install`. `use_frameworks!` (dynamic) + `post_install` hook 자동 적용.
   - 근본 해결: `iOSPostBuildProcessor.cs` 에 Podfile 후처리 자동화 (선택). [[project-podfile-manual-fixes]]

### 대기 중 (시간 지나면 자동 해결)
- **iOS v1.0.1 스토어 반영 (07-24 10:34 부터)** — 심사 통과 완료, 시각 도달 시 자동 배포
- Play upload key reset NO.1·NO.2 (2~5 영업일, 07-22 밤 신청)
- NO.1 Play v1.0 심사 (07-18 재업로드 후 6일차 이상)
- AdMob 실제 광고 iOS 새 앱 검증 (24~72h) — TestFlight/App Store 첫 실행 시 no-fill 정상
- Firebase Crashlytics 세션 카운트 임계치 도달
- iOS Test ad ID 분기 (`AdManager.cs:15-16`) — 현재 Android test ID 만 하드코딩. 나중 Development Build 로 iOS 실기 테스트 시 iOS test ID 로 분기 필요.
...
- **메뉴→레벨선택 리셋 우회 차단 (`AdManager.PendingRewardedOnLevelEnter`)**: 인게임에서 메뉴→레벨선택→아무 레벨 재진입으로 광고 없이 리셋되는 허점을 static bool 플래그로 차단. `GameManager.HandleMenuLevelSelect` 가 pour 이력 있을 때만 세팅 (`_resetButton.interactable` 재사용), `GameManager.Start` 마지막에서 소비 + `ShowRewarded` 호출. process 스코프라 앱 재시작 시 자동 리셋 (PlayerPrefs 아님). LS→Game 진입 시점에 게이팅하는 이유: `LevelSelectManager` 는 UI asmdef 라 Game(AdManager) 참조 불가 (circular) → LS 씬 안 건드리고 GameManager 안에서 signal 왕복. 새 광고 지점 추가 시 이 패턴 참고.
...
- **Editor Firebase 네이티브 미탑재 → 모든 동기 호출 try-catch 필수**: macOS Editor 에 `FirebaseCppApp-13_13_0.bundle` 없음. `FirebaseAnalytics.LogEvent` 같은 동기 호출 시 `DllNotFoundException` → 호출한 메서드 중단 → 그 뒤 로직 실행 안 됨 (예: 클리어 팝업 안 뜸). 신규 Firebase 동기 호출 추가 시 반드시 try-catch. Async 호출은 `ContinueWithOnMainThread` 안에서 예외 격리됨. 실기(iOS/Android) 는 정상 동작하므로 try 블록에서 그대로 실행. [[project-editor-firebase-missing-native]]
- **Editor 광고는 씬 전환과 충돌**: `AdManager.ShowRewarded` 가 리셋(씬 리로드) 먼저 실행 후 광고 Show 순서인데, Editor AdMob 시뮬레이터가 씬 destroy 도중 뜨면서 input lock 발생. `#if UNITY_EDITOR` 로 광고 Show 자체 skip. 실기엔 원래 순서 (리셋 → 광고) 그대로. 전면 광고(`ShowInterstitial`)도 같은 이슈 발생 가능하지만 현재 Editor 검증에서 재현 안 돼 방치 (필요 시 동일 방식 적용).
- **App Store 심사 시 "어린이용 (Kids Category)" 절대 금지**: Apple 이 Kids Category 앱에 COPPA 강제 → AdMob 실질 불가 (개인화 광고 금지, 서드파티 추적 금지) + Firebase Analytics 제약. 우리 앱은 광고 있음 + IDFA 추적 → 어린이용 선택 시 심사 즉시 반려. "해당 없음" 선택. 연령 등급 4+ 자체는 상관 없음 (자동 계산 결과).
- **App Store 심사 페이지 데이터 수집 신고**: Play Console 신고와 반드시 일관. 5개 카테고리 (기기 ID / 제품 상호작용 / 광고 데이터 / 충돌 데이터 / 실적 데이터). 사용자 신원 연결 = 모두 "예" (Firebase pseudonymous ID 로 자동 연결). 트래킹 = "예" 인 건 기기 ID + 광고 데이터 둘 뿐 (AdMob 광고 개인화). 나머지는 트래킹 "아니오". 이 조합이 실제 앱 동작 (`NSUserTrackingUsageDescription` + ATT 팝업) 과 일치해야 Apple 자동 스캔 통과.
- **App Store 심사 페이지 "광고 없이" 문구 금지**: 설명·부제·프로모션 텍스트 어디에도 "광고 없음" 뉘앙스 금지. 3라운드마다 전면광고 + 리셋 보상형 광고 있음. 심사 반려 or 리뷰 폭탄 리스크. 예: "광고 스트레스 없이 조용히" 같은 문구도 X. Play Console 도 광고 있음으로 신고했으니 일관 유지.
- **App Store 앱 지역 배포는 대한민국만**: UMP SDK (GDPR 동의 UI) 안 넣었으므로 EU/글로벌 배포 시 AdMob 계정 정지 + GDPR 벌금 리스크. Play Console 도 동일. 글로벌 확장 시 UMP 통합 필수 (별도 재심사).
