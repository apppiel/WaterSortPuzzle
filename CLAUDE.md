...
### 최근 완료 (2026-07-15)
- **리워드 코드 시스템 인프라 완료** (로직·Firebase 검증 성공, UI만 남음)
  ...
  - UI는 `OnCodeIssued` 이벤트로 분리 (NO.1은 UI Toolkit 하드코딩, NO.2는 uGUI 팝업이 이벤트 구독 예정)
  - Editor Play 모드에서 실제 Firestore 문서 발급/저장 검증 완료

### 다음 세션 즉시 할 일
1. **리워드 UI 팝업** (uGUI) — 로직 완성됐으니 껍데기만 남음
   - NO.1의 `Assets/UI/RewardPanel.uxml/uss` 참고 (구성: 오버레이 + 카드 + 코드 텍스트 + 복사·계속 버튼)
   - `RewardManager.OnCodeIssued` 이벤트 구독 → 코드 표시
   - 브랜드 컬러 적용 (`#FFC2C2` / `#FF8C8C`)
   - `GameManager.BuildRewardManager()` 내부의 임시 Debug.Log 리스너를 UI 팝업으로 교체
2. **릴리즈 빌드 + 갤럭시 실기 검증** (리워드 end-to-end)
   - Level 100 실제 클리어 → 팝업에 코드 표시되는지 → Firestore에 실기 device ID로 문서 새로 생기는지
   - 동시에 (a) 오버레이 배너 사라졌는지 (b) UI pink (c) 실광고 확인
...
- [ ] uGUI 팝업 UI (다음 세션) — `OnCodeIssued` 이벤트 구독, 코드 표시 + 복사 버튼 + 계속 버튼
- [ ] 실기(갤럭시) end-to-end 검증 — 실제 device ID로 Firestore 문서 생성 확인
